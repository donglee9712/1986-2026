...
const getTimingByViewport = () => {
  if (window.matchMedia('(max-width: 720px)').matches) {
    return { charStagger: 0.05, charDuration: 0.2, itemGap: 0.2 };
  }
  if (window.matchMedia('(max-width: 1200px)').matches) {
    return { charStagger: 0.06, charDuration: 0.22, itemGap: 0.15 };
  }
  return { charStagger: 0.07, charDuration: 0.25, itemGap: 0.1 };
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const { charStagger, charDuration, itemGap } = getTimingByViewport();
  let cumulativeDelay = 0;

  MENU_ITEMS.forEach((item, idx) => {
    const el = createMenuItem({
      ...item,
      image: pickRandomIcon(),
      index: idx,
    ...
    cumulativeDelay += charDuration + Math.max(0, charCount - 1) * charStagger + itemGap;
  });
