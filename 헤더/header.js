(function () {
  const BUSINESS_INQUIRY_URL = "https://www.1986.co.kr/ask";

  const resolveWorkUrl = () => {
    const navContainer = document.querySelector("[data-coco-nav]");
    const rootAttr = navContainer?.getAttribute("data-nav-root") || "./";
    const normalizedRoot = rootAttr.endsWith("/") ? rootAttr : `${rootAttr}/`;
    return `${normalizedRoot}portfolio.html`;
  };
  ...
  const ensureWorkLinks = () => {
    ...
    const workUrl = resolveWorkUrl();
    document.querySelectorAll("a").forEach((link) => {
      const label = normalize(link.textContent);
      if (!["work", "워크", "portfolio", "포트폴리오"].includes(label)) return;
      link.href = workUrl;
      link.removeAttribute("target");
      link.removeAttribute("rel");
    });
